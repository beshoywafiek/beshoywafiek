  ...
  (function(){
    ...
    (function(){
      ...
      // Watch the container, never the cards themselves: a card in its hidden state is
      // clipped to zero height, so observing it directly reports ratio 0 forever and it
      // would never be told to appear.
      var host = document.querySelector('#work .carousel-wrap') || document.getElementById('work');
      if(!host) return;
      var staged = Array.prototype.slice.call(cards, 0, 8);
      function release(){
        staged.forEach(function(card, i){
          setTimeout(function(){ card.classList.remove('pre-reveal'); }, (i % 4) * 110);
        });
      }
      ...
      setTimeout(release, 2500); // safety net, should never be the thing that fires
    })();
  ...
  // ===== Keep the work carousel from ever showing a blank card =====
  // 12 of these images come from an external host, and lazy loading only starts a
  // fetch once a card is nearly in view — in a horizontal carousel that means every
  // swipe lands on images that have not started downloading yet. So: mark cards as
  // loaded, warm the rest quietly in the background, and jump the queue on interaction.
  (function(){
    var imgs = Array.prototype.slice.call(document.querySelectorAll('#work .card img'));
    if(!imgs.length) return;

    function markLoaded(img){
      var box = img.closest ? img.closest('.card-img') : null;
    ...
    var queue = imgs.filter(function(img){ return img.getAttribute('loading') === 'lazy'; });
    function warmNext(count){
      var warmed = 0;
      while(queue.length && warmed < count){
        var img = queue.shift();
        if(img.complete && img.naturalWidth > 0) continue;
        img.setAttribute('loading', 'eager'); // starts the fetch straight away
        warmed++;
      }
    }

    // Background trickle: one at a time, well after the page itself has settled.
    var timer = null;
    function startTrickle(){
      if(timer) return;
      timer = window.setInterval(function(){
        if(!queue.length){ window.clearInterval(timer); timer = null; return; }
        warmNext(1);
      }, 220);
    }
    if(document.readyState === 'complete') window.setTimeout(startTrickle, 900);
    else window.addEventListener('load', function(){ window.setTimeout(startTrickle, 900); });

    // The moment the visitor touches the carousel, pull the next few forward.
    var viewport = document.querySelector('.carousel-viewport');
    var prevBtn = document.getElementById('carPrev');
    var nextBtn = document.getElementById('carNext');
    function rush(){ warmNext(6); startTrickle(); }
    if(viewport){
      viewport.addEventListener('pointerdown', rush, {passive: true});
      viewport.addEventListener('wheel', rush, {passive: true});
    }
    if(prevBtn) prevBtn.addEventListener('click', rush);
    if(nextBtn) nextBtn.addEventListener('click', rush);
  })();
